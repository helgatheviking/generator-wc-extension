<?php
/**
 * <%= props.classPrefix %>_Helpers class
 *
 * @class 	<%= props.classPrefix %>_Order
 * @version 0.1.0
 * @since   0.1.0
 */
...
class <%= props.classPrefix %>_Helpers {
	...
	public static function is_product_checkbox( $product_id ) {
		if ( 'yes' == get_post_meta( $product_id, '_<%= props.funcPrefix %>', true ) ) {
			return true;
		} else {
			return false;
		}
	...
	public static function get_sample_number( $product_id ) {
		return floatval( get_post_meta( $product_id, '_<%= props.funcPrefix %>_number', true ) );
	}
	...
	public static function get_sample_textbox( $product_id ) {
		return get_post_meta( $product_id, '_<%= props.funcPrefix %>_textbox', true );
	}
